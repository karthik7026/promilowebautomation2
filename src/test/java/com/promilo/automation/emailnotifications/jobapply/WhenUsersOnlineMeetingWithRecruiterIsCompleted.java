...
import com.promilo.automation.advertiser.mymeetings.MymeetingsHostFunctionality;
import com.promilo.automation.pageobjects.signuplogin.JobListingPage;
	...
	public class WhenUsersOnlineMeetingWithRecruiterIsCompleted extends Baseclass {
	   ...
	   public void applyForJobAsRegisteredUser(Page page, String inputvalue, String password, String name, String otp, String mailphone) throws Exception {
	       ...
	       Page page1 = initializePlaywright();
	       page1.navigate(prop.getProperty("stageUrl"));
	       page1.setViewportSize(1000, 768);

	       Thread.sleep(5000);

	       AdvertiserLoginPage login = new AdvertiserLoginPage(page1);
	       ...
	       AdvertiserHomepage myaccount = new AdvertiserHomepage(page1);
	       myaccount.myAccount().click();

	       AdverstiserMyaccount prospect = new AdverstiserMyaccount(page1);
	       prospect.myProspect().click();

	       AdvertiserProspects approveFunctionality = new AdvertiserProspects(page1);
	       approveFunctionality.Jobs().click();

	       Thread.sleep(3000);

	       approveFunctionality.ApproveButton().first().click();
	       page1.locator("//button[contains(text(),'Done')]").click();
	       
	       
	       
AdvertiserMymeetingPage myMeeting= new AdvertiserMymeetingPage(page1);
page1.locator("//span[text()='My Meeting']").click();
...
myMeeting.joinMeetingButton().click();
	       ...
	       page2.locator("//p[text()='Confirmation of Accepted Meeting Request - UST Global, Bengaluru/Bangalore']").first().click();
	       
	       Page newPage = page1.waitForPopup(() -> {
		       page2.locator("//button[contains(text(),'Join')]").click();
	       });
	       
	       
	       newPage.locator("//span[text()='Join Now']").click();
	       newPage.locator("rect[width=\"24.979\"]").click();
	       newPage.locator("//*[name()=\"path\" and contains(@d,\"M22.359 17\")]").click();
	       newPage.locator("//button[text()='Join now']").click();
	       newPage.locator("//div[contains(@class,\"rounded-lg bg-red-150 border-2 border-transparent border-solid md:m-2 m-1\")]//div[contains(@class,\"rounded-lg\")]//*[name()=\"svg\"]").click();
	       newPage.locator("//button[text()='Yes']").click();
	       newPage.locator("[name='remark']").fill("Good");
	       
	    // Locate all stars
	       Locator star = page.locator("//span[text()='★']");

	       // Loop through first 3 stars
	       for (int i = 0; i < 3; i++) {
	    	   star.nth(i).click();
	       }
