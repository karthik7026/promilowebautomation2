...
public class UserRequestIsAcceptedByTheAdvertiser extends Baseclass {
   ...
   public void applyForJobAsRegisteredUser(Page page, String inputvalue, String password, String name, String otp, String mailphone) throws Exception {
    ...
    // Select today's date dynamically
       ...
       Locator firstEmail = page2.locator("//p[contains(text(),'Confirmation of Accepted Meeting Request ')]").first();
       
       firstEmail.waitFor(new Locator.WaitForOptions().setState(WaitForSelectorState.VISIBLE));

       firstEmail.click();
       System.out.println(" mail oepned");
       page2.locator("//p[contains(text(),'I trust this email finds you well. I am')]").textContent();
       page2.locator("(//table)[12]").textContent();
       page2.locator("//p[contains(text(),'Your journey with Promilo is')]");
       page2.locator("//p[contains(text(),'We’re cheering you on as you')]").textContent();
       
       
       page.locator("//button[contains(text(),'Join')]").click();
       page2.locator("//span[text()='Reschedule']").click();
